...
class TestCases(unittest.TestCase):
    ...
    @staticmethod
    def try_(ride_id, date ,time, licenseNo, fro, destination, fare ):
...
#test for adding a single ride offer
    def test_addRide(self):
        ...
        data = json.dumps(self.try_(1, "10/02/2018", "10pm", "Mukono","iganga", 10000))

        response = self.test_client.post('/api/v1/rides/create', data=data, headers=self.json_headers)
        result = json.loads(response.data.decode())

        self.assertEqual(result, {'ride': self.try_(1, "10/02/2018", "10pm", "Mukono","iganga", 10000)})
...
#test for returning all ride offers
        ...
        if type(response.data) == str:
            ...
            self.assertEqual(results, {"rides": [self.try_(1, "10/02/2018", "10pm", "Mukono","iganga", 10000)]})
            self.assertEqual(response.status_code, 200)
...
#test for deleting a ride offer
    ...
    def find_sample_ride(self, ride_id):
        ...
        return self.try_(ride_id, "4", "10/02/2018", "10pm", "Mukono","iganga", 10000)
